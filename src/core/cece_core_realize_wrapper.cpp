...
/**
 * @brief Core realize function (implemented in cece_core_realize.cpp)
 * This is the actual implementation that only needs data_ptr and rc.
 */
void cece_core_realize_impl(void* data_ptr, int* rc);

/**
 * @brief 2-parameter wrapper for cece_core_realize (called by Fortran cap).
 *
...
void cece_core_realize(void* data_ptr, int* rc) {
    cece_core_realize_impl(data_ptr, rc);
}
...
void cece_core_realize_with_states(void* data_ptr, void* importState_ptr, void* exportState_ptr, void* grid_ptr, int* rc) {
    ...
    if (exportState_ptr == nullptr) {
        std::cerr << "ERROR: cece_core_realize_with_states - exportState_ptr is null" << std::endl;
        if (rc != nullptr) {
            *rc = -1;
        }
        return;
    ...
    if (grid_ptr == nullptr) {
        std::cerr << "ERROR: cece_core_realize_with_states - grid_ptr is null" << std::endl;
        if (rc != nullptr) {
            *rc = -1;
        }
        return;
