...
from plotting import VariableScale, derive_scales, render_all_plots, render_combo_plots, render_spatial_plot
...
def _stats_frame(rows: list[tuple[str, float, float]]) -> pd.DataFrame:
    return pd.DataFrame([{"variable": variable, "min": vmin, "max": vmax} for variable, vmin, vmax in rows])


def test_derive_scales_exact_suite_wide_min_max() -> None:
    stats = _stats_frame([("co", 0.2, 0.9), ("co", 0.1, 0.5), ("co", 0.3, 2.9), ("nox", -1.0, 1.0)])
...
def test_render_spatial_plot_writes_png(tmp_path: Path, combo_dirs) -> None:
    ...
    scale = VariableScale(variable="co", vmin=float(first.min()), vmax=float(first.max()))
    render_spatial_plot(root / "3f9a1c2b7d4e8a01" / "cece_20100101_010000.nc", "co", scale, out_path)
...
def test_render_combo_plots_gif_disabled(combo_dirs) -> None:
    ...
    render_combo_plots(combo_dir, {"co": VariableScale(variable="co", vmin=0.0, vmax=3.0)}, gif_enabled=False)
    plots = combo_dir / "plots"
    assert (plots / "co__cece_20100101_010000.png").is_file()
...
def test_plotting_requires_descriptive_stats(tmp_path: Path, cece_config_path: Path) -> None:
...
def test_stats_disabled_with_plotting_disabled_is_valid(tmp_path: Path, cece_config_path: Path) -> None:
