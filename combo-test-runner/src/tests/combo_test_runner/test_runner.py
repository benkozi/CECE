...
def test_run_driver_success_writes_out(mocker: MockerFixture, tmp_path: Path) -> None:
    check_output = mocker.patch("runner.subprocess.check_output", return_value=b"driver ok\n")
    out_path = tmp_path / "x.out"

    run_driver(_settings(), container_yaml=PurePosixPath("/work/x.yaml"), out_path=out_path, timeout_s=7)
...
def test_run_driver_nonzero_exit_writes_out_and_reraises(mocker: MockerFixture, tmp_path: Path) -> None:
...
def test_run_driver_timeout_writes_out_and_reraises(mocker: MockerFixture, tmp_path: Path) -> None:
