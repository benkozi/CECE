...
def _maccity_sweep() -> Sweep:
    return Sweep(
        cece_data=CeceDataSweep(
            streams=[
                StreamSweep(name="MACCITY", mapalgo=[Mapalgo.bilinear, Mapalgo.consd, Mapalgo.passthrough])
            ]
        )
    )


def test_maccity_sweep_enumerates_three_qualified_combos(base_config: CeceConfig) -> None:
...
def test_normalization_declaration_order_never_matters(base_config: CeceConfig) -> None:
    ...
    tidy = Sweep(
        species={"co": [SpeciesEntrySweep(operation=[Operation.add, Operation.replace])]},
        cece_data=CeceDataSweep(streams=[StreamSweep(name="MACCITY", mapalgo=[Mapalgo.bilinear, Mapalgo.consd])]),
    )
    shuffled = Sweep(
        cece_data=CeceDataSweep(streams=[StreamSweep(name="MACCITY", mapalgo=[Mapalgo.consd, Mapalgo.bilinear])]),
        species={"co": [SpeciesEntrySweep(operation=[Operation.replace, Operation.add])]},
    )
    tidy_combos = enumerate_combos(tidy, base_config)
    shuffled_combos = enumerate_combos(shuffled, base_config)
    assert [combo.name for combo in tidy_combos] == [combo.name for combo in shuffled_combos]
    assert [combo.combo_id for combo in tidy_combos] == [combo.combo_id for combo in shuffled_combos]
...
def test_build_config_applies_to_named_non_first_stream(two_stream_config_path: Path) -> None:
    ...
    config = build_config(combo, output_directory=".", config_path=two_stream_config_path)
...
def test_build_config_supplies_vdist_companion_fields(base_config: CeceConfig, cece_config_path: Path) -> None:
    sweep = Sweep(species={"co": [SpeciesEntrySweep(vdist_method=[VdistMethod.height, VdistMethod.pressure])]})
...
def test_unknown_stream_selector_rejected(base_config: CeceConfig) -> None:
    sweep = Sweep(cece_data=CeceDataSweep(streams=[StreamSweep(name="NOPE", mapalgo=[Mapalgo.consd])]))
...
def test_oversized_species_entry_list_rejected(base_config: CeceConfig) -> None:
    sweep = Sweep(
        species={"co": [SpeciesEntrySweep(operation=[Operation.add]), SpeciesEntrySweep(operation=[Operation.replace])]}
    )
    with pytest.raises(ValueError, match="entry blocks"):
        enumerate_combos(sweep, base_config)


def test_write_combos_csv_dereferences_directories(tmp_path: Path, base_config: CeceConfig) -> None:
    combos = enumerate_combos(_maccity_sweep(), base_config)
    csv_path = tmp_path / "combos.csv"
    frame = write_combos_csv(combos, run_id="01JZZZZZZZZZZZZZZZZZZZZZZZ", csv_path=csv_path)

    assert csv_path.is_file()
    assert list(frame.columns) == ["run_id", "combo_id", "name", "target", "field", "value"]
    ...
    assert (consd["target"].iloc[0], consd["field"].iloc[0], consd["value"].iloc[0]) == (
        "MACCITY",
        "mapalgo",
        "consd",
    )
