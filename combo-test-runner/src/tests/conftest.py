...
def pytest_sessionstart(session: pytest.Session) -> None:
    ...
    try:
        suite = SuiteConfig.from_yaml(
            suite_path, config_search_path=settings.config_search_path
        )
    ...
    try:
        ...
        resolved_baselines = resolve_baseline_comparisons(
            suite.baseline_comparisons, combos
        )
    except ValueError as exc:
        raise pytest.UsageError(str(exc)) from exc
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    if "driver_run" in metafunc.fixturenames:
        ...
        metafunc.parametrize(
            "driver_run", combos, ids=[combo.name for combo in combos], indirect=True
        )
