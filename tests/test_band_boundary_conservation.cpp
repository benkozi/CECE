...
// mirrors the KokkosMpiEnvironment pattern in test_cece_utils.cpp. Init must
// NOT happen during `--gtest_list_tests`: gtest discovery runs this binary as
// a forked child of the ctest step, where a late MPI_Init aborts (the step's
// PMI endpoint serves only the first MPI program).
